...
`users`、`roles`、`departments`、`menus` 四张核心权限表的**单行增删改**自动接入业务审计：脚手架通过 GORM 插件（`shared/pkg/audittrail`）在写事务内生成变更前后快照，**审计与业务变更同一事务提交——审计写入失败则业务一并回滚**（fail closed），不会出现"权限改了但没留痕"的窗口。

- **范围**：四张表本体的单行增删改；关联表与 Raw SQL 不在本批覆盖范围。
- **脱敏**：密码、TOTP、恢复码等敏感字段在快照里**递归脱敏**，不落明文。
- **归因**：快照记录操作者、租户与变更前后 JSON，前端「审计日志」页（关键词 + 动作 + 目标类型）直接可查。
- **与操作日志的区别**：操作日志走 HTTP 中间件异步 best-effort（写库失败不影响业务）；这套事务审计是**同事务强一致**——适合"改权限必须可追溯、不可丢失"的场景。

## 租户隔离

审计记录创建时写入**请求租户**，列表 / 汇总 / 筛选统一限制在当前租户——管理端读不到其他租户的审计数据，也不会把事件误归默认租户。自动保留策略的清理按运维维度跨全部租户执行，读取始终受租户约束。
