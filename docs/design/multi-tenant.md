...
相关：[`../PRODUCT_LINES.md`](../PRODUCT_LINES.md)
...
| **M5** | ✅ 公告 `notices` 补 `tenant_id` 与隔离（WS 推送按票据租户）；系统设置 `/system-settings` 加平台管理员门禁（含 AI/SMTP 密钥）；租户停用即踢下线（refresh 拒绝停用租户 + 停用租户禁止登录，一个 access-token TTL 内失效）；默认租户禁止停用 |
...
- 公告租户化：`000016_add_tenant_id_notices.sql`（存量归默认租户）
...
| 2026-07-18 | M5：公告租户化、系统设置平台管理员门禁、租户停用踢下线、默认租户锁定；顺带修网关 identity 路由漏 `/tenants`（租户管理页此前经网关 404） |
