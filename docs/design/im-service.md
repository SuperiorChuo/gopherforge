...
> 状态：**实验特性 🧪** · 设计草案；**M1～M4 已落地**（`microservices/services/im`：内核 + 埋码 + 技能组/排队/转接 + 机器人预答/转人工/小结）  
> 范围：`microservices` 产品线新增 `im-service` + 管理台坐席模块 + **网页埋码 Widget**  
> 原则：完全自研消息面；与 identity/auth/file/ai 解耦协作；单体线不实现完整 IM。  
> **发版**：不纳入近期 `v0.1.0` 承诺范围；协议与表结构可能变。
...
## 14. 开放问题（已全部拍板，2026-07-17）

1. 域表 migration 放 `im` 自管还是并入 `monitor` 统一链？**✅ im 自管（AutoMigrate）**；转正时并入 goose（见 `data-layer-boundaries.md`）。  
2. 消息是否软删除 / 保留时长（合规）？**✅ 硬删除 + 可配置保留期**：`IM_RETENTION_DAYS`（默认 180，0=永久），每日 job 删超期 closed 会话的消息与附件对象（MinIO/本地级联）。不做行级软删除——数据滞留对合规反而不利。  
3. 是否第一期就上多租户 `tenant_id` 字段预留？**✅ 已上**（多租户 M3 已隔离）。  
4. Widget 域名用独立子域还是与网关同域路径？**✅ 网关同域路径（`/im/` PathPrefix）**：当前单栈/内网形态下独立子域只增加 DNS+证书成本；未来有 CDN 需求时再迁，widget.js 的 `data-base` 属性已预留切换能力。  
