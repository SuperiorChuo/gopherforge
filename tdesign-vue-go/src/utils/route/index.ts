...
function asyncImportRoute(routes: RouteItem[] | undefined) {
  ...
  routes.forEach(async (item) => {
    ...
    if (component) {
      ...
      // 如果是子菜单容器（无组件但有子菜单），自动重定向到第一个子菜单
      if (children && children.length > 0) {
        (item as any).redirect = children[0].path;
      }
    }
...
export function transformObjectToRoute<T = RouteItem>(routeList: RouteItem[]): T[] {
  routeList.forEach(async (route) => {
    ...
    if (component) {
      ...
      // Submenu container: has children but no component - use PARENT_LAYOUT
      route.component = PARENT_LAYOUT;
    } else {
