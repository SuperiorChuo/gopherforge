...
export class VAxios {
  ...
  request<T = any>(config: AxiosRequestConfigRetry, options?: RequestOptions): Promise<T> {
    ...
    const throttleOptions = requestOptions.throttle;
    const debounceOptions = requestOptions.debounce;

    if (throttleOptions !== undefined && debounceOptions !== undefined) {
      throw new Error('throttle and debounce cannot be set at the same time');
    }

    if (throttleOptions && throttleOptions.delay !== 0) {
      return new Promise((resolve) => {
        throttle(() => resolve(this.synthesisRequest(config, options)), throttleOptions.delay);
      });
    }

    if (debounceOptions && debounceOptions.delay !== 0) {
      return new Promise((resolve) => {
