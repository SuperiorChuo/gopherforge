...
// GetForUserContext loads one conversation owned by userID.
...
func (d *ConversationDAO) ListForUserContext(ctx context.Context, userID uint, req pagination.PageRequest) ([]model.AIConversation, int64, error) {
	...
	query := d.dbWithContext(ctx).Model(&model.AIConversation{}).Where("user_id = ?", userID)
...
// DeleteForUserContext deletes one conversation owned by userID. Messages
// cascade at the database level. gorm.ErrRecordNotFound is returned when the
...
// TouchContext bumps a conversation's updated_at.
