...
func (a *NotificationAPI) CreateTicket(c *gin.Context) {
	...
	ticket, err := jwt.GenerateWebSocketTicketWithTenant(userID.(uint), usernameString(username), tenantID, time.Minute)
	if err != nil {
...
func (a *NotificationAPI) Connect(c *gin.Context) {
	...
	// 用票据里的租户查激活公告（NoticeDAO 按 ctx 租户过滤）
	noticeCtx := context.WithValue(c.Request.Context(), middleware.TenantIDContextKey, jwt.NormalizeTenantID(claims.TenantID))
	a.sendActiveNoticeMessages(noticeCtx, conn)
