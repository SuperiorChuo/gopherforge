...
func (a *UserAPI) RefreshToken(c *gin.Context) {
	...
	// Reject refresh for a suspended tenant: disabled tenants lose access
	// within one access-token TTL, no global session store needed.
	if claims, perr := jwt.ParseRefreshClaimsContext(c.Request.Context(), req.RefreshToken); perr == nil {
		if terr := a.userService.EnsureTenantActiveContext(c.Request.Context(), jwt.NormalizeTenantID(claims.TenantID)); terr != nil {
			response.UnauthorizedWithCode(c, response.ErrorCodeAuthTokenInvalid, "tenant is disabled")
			return
		}
	}
