...
func RegisterProtectedRoutesWithDeps(r gin.IRoutes, deps sharedapi.Dependencies) {
	...
	loginSecurityAPI := NewLoginSecurityAPI(deps.Redis)
	r.GET("/login-security/blocked-ips", middleware.PermissionMiddleware("system:login-security:list"), loginSecurityAPI.ListBlockedIPs)
	r.DELETE("/login-security/blocked-ips/:ip", middleware.PermissionMiddleware("system:login-security:manage"), loginSecurityAPI.UnblockIP)
