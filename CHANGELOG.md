...
- **文档站曜石极光玻璃视觉升级**：中文与英文首页采用与控制台一致的靛蓝、紫色和青色品牌体系，新增架构状态 Hero、交付路径与终端视觉；导航、搜索、侧栏、功能卡、正文、代码块、表格和提示块统一为明暗双主题液态玻璃，并适配 390px 窄屏与减少动效偏好。
- **文档站细节打磨**：亮暗切换改为渐变双层交叉淡化不再跳变；新增全站自定义滚动条、代码块 hover 亮边与玻璃化复制按钮、引用块玻璃卡、渐变分割线与列表标记、表格渐变表头、正文标题品牌标记与目录指示条、页脚渐变线；首页“找到你需要的答案”升级为三列玻璃卡、截图展示升级为玻璃卡片表格，Hero 增加版本徽章、终端增加闪烁光标、能力卡 hover 图标发光；补充 Open Graph 分享元信息。
- **文档站可读性与玻璃质感强化**：修复亮色主题代码块深字深底看不清的问题（两主题统一亮色终端文字，纯文本对比度 15.65:1、语法高亮 7.6:1 以上）；代码块升级为深色玻璃材质（半透明底、背景模糊、顶部渐变高光线）；aurora 光斑浓度与尺寸提升、网格加密、玻璃容器高光强化；Hero/能力卡/交付路径/正文间距节奏重排并同步移动端。
- **文档站可信度与工程化优化**：建立版本、服务数、镜像数、端口、Compose profile 与首页命令的单一事实源，修复 React Router、Vite 端口、monitoring profile、arm64 版本等公开文档漂移；Hero 模拟状态改为真实仓库事实，版本和 Demo 数据边界前置，上手路径与资源入口合并，截图跟随站点主题；补齐中英文 canonical/hreflang/Open Graph/JSON-LD 与 1200×630 分享图，降低长文和移动端滤镜负担，新增事实、SEO 与 7 档浏览器回归门禁。
...
- **BPM 设计器职责拆分（同步自主项目）**：卡片链、表单编辑器、分支配置与节点配置抽为独立组件，保留既有流程状态模型。
- **前端路由 CSS 分包（同步自主项目）**：认证、仪表盘、监控、日志与文件样式随路由并行加载，入口 CSS 从 202054B 降至 106977B；公共动效不再依赖业务样式。
- **shared 大文件职责拆分（同步自主项目）**：审计插件按变更生命周期、主键边界和快照持久化拆分，数据权限按解析入口、部门树缓存与角色持久化拆分。
- **服务配置职责拆分（同步自主项目）**：六个基础服务的配置类型、默认值、环境覆盖和安全校验拆为独立同包文件，保留全部配置键、加载顺序与校验语义。
- **BPM 引擎职责拆分（同步自主项目）**：审批引擎按发起幂等、任务动作、实例生命周期、节点推进、审批人解析和持久化辅助拆分，保持事务与状态机语义不变。
...
- **邮件运行时配置读取器收敛（同步自主项目）**：system/monitor 复用 shared `CachedSettingReader`，保留租户覆盖、安全策略、模板和收件人语义。
...
- **列表查询重复重载**：`useTableQuery` 稳定等值参数对象与错误回调引用，避免页面传对象字面量或内联失败通知时无限重载、旧响应持续失效。
