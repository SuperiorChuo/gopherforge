...
| 🔌 **可扩展** | 微服务按域拆分 + 网关标签接入，适合后续挂载 AI / IM / 客服 / 呼叫中心等业务域。 |
...
| ☎️ 呼叫中心 | ⭐⭐⭐⭐ | 适合独立 `cc-service`（话单、坐席、IVR 对接），管理端仍用本仓 React 后台 |
...
| 规划项 | 说明 |
|--------|------|
| ☎️ 话单管理 | 呼入/呼出记录、录音索引、质检标记 |
| 👤 坐席台 | 签入签出、示忙示闲、技能组 |
| 🔗 CTI 对接 | 对接主流云呼叫 / SIP（以适配层隔离厂商差异） |
| 📊 呼叫报表 | 接通率、通话时长、放弃率、坐席效能 |
| 🔁 全渠道汇聚 | 电话 + IM + 小程序留言统一客户视图 |

> 建议：`cc-service` 独立进程，管理 UI 仍在本仓 React 控制台。
